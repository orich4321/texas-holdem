...
export type FiveCardHandCategory = 'high-card' | 'one-pair' | 'two-pair' | 'three-of-a-kind' | 'straight' | 'flush' | 'full-house';
...
export function evaluateFiveCardHand(cards: readonly Card[]): FiveCardHandEvaluation {
  ...
  throw new Error('Unsupported hand category: only high-card, one-pair, two-pair, three-of-a-kind, straight, flush, and full-house are supported');
}
...
export function compareFiveCardHands(left: readonly Card[], right: readonly Card[]): -1 | 0 | 1 {
  ...
  const categoryDifference =
    (leftEvaluation.category === 'full-house' ? 6 : leftEvaluation.category === 'flush' ? 5 : leftEvaluation.category === 'straight' ? 4 : leftEvaluation.category === 'three-of-a-kind' ? 3 : leftEvaluation.category === 'two-pair' ? 2 : leftEvaluation.category === 'one-pair' ? 1 : 0)
    - (rightEvaluation.category === 'full-house' ? 6 : rightEvaluation.category === 'flush' ? 5 : rightEvaluation.category === 'straight' ? 4 : rightEvaluation.category === 'three-of-a-kind' ? 3 : rightEvaluation.category === 'two-pair' ? 2 : rightEvaluation.category === 'one-pair' ? 1 : 0);
